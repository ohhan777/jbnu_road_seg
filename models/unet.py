...
class Unet(nn.Module):
    ...
    def forward(self, x: torch.Tensor):
        ...
        x = self.encoder(x)
        x = [feature.contiguous() for feature in x]  # Ensure all encoder outputs are contiguous
        x.reverse()
        x = self.decoder(x)
...
class DecoderBlock(nn.Module):
    ...
    def forward(self, x, skip: Optional[torch.Tensor] = None):
        ...
        if skip is not None:
            x = torch.cat([x, skip], dim=1).contiguous()  # Ensure concatenation output is contiguous
        x = self.conv1(x).contiguous()  # Ensure conv1 output is contiguous
        x = self.conv2(x).contiguous()  # Ensure conv2 output is contiguous
        return x
...
class UnetDecoder(nn.Module):
    ...
    def forward(self, x: List[torch.Tensor]):
        encoder_head = x[0].contiguous()  # Ensure encoder head is contiguous
        skips = [feature.contiguous() for feature in x[1:]]  # Ensure all skip connections are contiguous
        x = self.center(encoder_head).contiguous()  # Ensure center output is contiguous
        for i, b in enumerate(self.blocks):
            skip = skips[i] if i < len(skips) else None
            x = b(x, skip).contiguous()  # Ensure block output is contiguous
        x = self.final_conv(x).contiguous()  # Ensure final output is contiguous
        return x
...
def main():
    ...
    for param in model.parameters():
        param.data = param.data.contiguous()  # Ensure all parameters are contiguous
        if param.grad is not None:
            param.grad.data = param.grad.data.contiguous()  # Ensure all gradients are contiguous
